 ...
 * 
 */
class Izin_non_full_day extends REST_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('pengajuan/M_Izin_non_full_day');
	}
	...
	public function index_get()
	{
		...
		if ($nik_baru === null and $id === null) {
			$user = $this->M_Izin_non_full_day->get_index_non_full_day();
		} else {
			$user = $this->M_Izin_non_full_day->get_index_non_full_day($nik_baru, $id);
		}
	...
	public function index_post()
	{
		$data = [
			...
			'jenis_non_full' => $this->post('jenis_non_full'),
		...
		if ($this->M_Izin_non_full_day->createNon_full_day($data) > 0) {
			$this->response([
				'status' => true,
	...
	public function index_put()
	{
		...
		if ($this->M_Izin_non_full_day->updateApproval($data, $id) > 0 and $this->M_Izin_non_full_day->update_data($data2, $shift_day, $badgenumber, $status) > 0) {
			$this->response([
				'status' => true,
    ...
    {     
        ...
        if ($jabatan === null) {
            $team = $this->M_Izin_non_full_day->get_index_non_full_day_atasan();
        } else {
            $team = $this->M_Izin_non_full_day->get_index_non_full_day_atasan($jabatan);
        }
    ...
    {     
        ...
        $team = $this->M_Izin_non_full_day->get_index_full_day_atasan_lokasi($jabatan, $lokasi);
    ...
    {     
        ...
        $team = $this->M_Izin_non_full_day->get_index_non_full_day_feedback($nik_baru, $tanggal);
