...
class M_Supplier extends CI_Model
{
	...
	public function getBtbSupplier($mk_barcode = null)
	{
		$sql="SELECT 
			a.`mk_barcode`,
			a.`mk_pool`,
		    ...
		    ''
		      ...
		      FROM
			db_sc_modust.`tbl_sc_armada_barcode` a 
			LEFT JOIN db_sc_modust.`tbl_sc_armada_barcode_waktu` b 
		    ON a.`mk_barcode` = b.`mk_barcode` 
			LEFT JOIN db_sc_modust.`tbl_sc_jadwal_supply` c 
		    ON a.`mk_co_real` = c.`js_co` 
			LEFT JOIN db_sc_modust.`tbl_sc_po_depo` d 
		    ON a.`mk_po` = d.`po_po_old` 
			LEFT JOIN db_sc_modust.`tbl_hp3_depo` f 
		    ON a.`mk_co_real` = f.`co` 
			LEFT JOIN db_sc_modust.`tbl_hp3_pool` i 
		    ON a.`mk_co_real` = i.`co` 
			LEFT JOIN db_sc_modust.`tbl_sc_po_pool` g 
		    ON g.`po_po_new` = a.`mk_po` 
			LEFT JOIN db_sc_modust.`tbl_pabrikan_checker` h 
	...
	public function getBakkbPrint($asal_pengirim = null, $co = null)
	{
		$sql = "SELECT
                *
            FROM `db_sc_modust`.`tbl_tolakan_keluar_pool` a
            INNER JOIN `db_sc_modust`.`tbl_sc_armada_barcode` b
