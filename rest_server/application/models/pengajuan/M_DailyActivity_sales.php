...
class M_DailyActivity_sales extends CI_Model
{
  ...
  public function getPilih_kelurahanTVIP($szBranchId = null)
    {
        $sql = "SELECT 
          a.`szSubDistrict` 
        FROM
          `dms_gen_geotree_branch` a 
        WHERE a.`szBranchId` = '$szBranchId' ";
        $hasil = $this->db10->query($sql);
        return $hasil->result_array();
    }
  
  public function getCustomerRetail_TVIP($szSoldToBranchId = null, $szSubDistrict = null)
    {
        $sql = "SELECT 
                  a.`szId`,
                  a.`szName`,
                ...
                WHERE b.`szObjectId` = 'DMSCustomer' 
                  AND c.`szStatus` = 'ACT' 
                  AND d.`szSoldToBranchId` = '$szSoldToBranchId' 
                  AND e.`szclass` = 'Retail'
          AND b.`szSubDistrict` = '$szSubDistrict'";
        $hasil = $this->db10->query($sql);
        return $hasil->result_array();
    }
  
  public function getCustomerRetail_ASA($szSoldToBranchId = null, $szSubDistrict = null)
    {
        $sql = "SELECT 
                  a.`szId`,
                  a.`szName`,
